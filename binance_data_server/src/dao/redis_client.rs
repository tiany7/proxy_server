...
impl RedisClient {
    ...
    pub async fn modify_set_value(&self, zset_key: &str, score: f64, new_bardata: BarData) -> redis::RedisResult<()> {
        let mut conn = self.client.get_async_connection().await.unwrap();
        
        let existing_data: Option<Vec<u8>> = conn.zrangebyscore_limit(zset_key, score, score, 0, 1).await?;
        
        if let Some(data) = existing_data {
            let old_bardata = BarData::decode(&*data).unwrap();

            tracing::info!("Old BarData: id={}, name={}, value={}", old_bardata.id, old_bardata.name, old_bardata.value);
            ...
            trace::warn!("No data found for score {}", score);
            Ok(())
        }
    }
}